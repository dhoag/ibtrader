 ...
 */
public class CombinedScrollBar extends JScrollBar implements AdjustmentListener, AxisChangeListener {
	private static final long SCALER = 10000;
	private static final double EXTRA_MARGIN_FACTOR = 0.05; // factor above and
															// below the min/max
															// to produce some
															// margin
	...
	public CombinedScrollBar(CombinedDomainXYPlot combinedDomainPlot) {
		...
		for (Object subPlot : subPlots) {
			...
			for (int datasetNumber = 0; datasetNumber < datasetCount; datasetNumber++) {
				...
				for (int series = 0; series < seriesCount; series++) {
					...
					int[] itemBounds = RendererUtilities.findLiveItems(dataset, series, lowerBound,
							upperBound);
					int firstItem = itemBounds[0];
					int lastItem = itemBounds[1];

					// Extend the range of applicable data to be one point
					// before and after the range,
					// so that we get the lines (such as profit graph) that
					// extend off the edges
					if (!isOHLC) {
